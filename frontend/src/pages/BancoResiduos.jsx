                ...
                <tbody>
                  {residuos.map((r, idx) => (
                    ...
                    >
                      ...
                      <td style={{ padding: isMobile ? '9px 10px' : '12px 13px', color: 'var(--text-secondary)', fontSize: '.82rem', maxWidth: 160 }}>
                      ...
                      <td style={{ padding: isMobile ? '9px 10px' : '12px 13px', color: 'var(--text-secondary)', fontSize: '.82rem', maxWidth: 150 }}>
                        <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {r.creado_por_nombre || '—'}
                        </div>
                      </td>
        ...
        <div style={{ display: 'grid', gap: 20 }}>
          ...
          <div style={{
            ...
            {(() => {
              ...
              const nDisp = g.disponibles || 0;
              // Largo PROMEDIO real de un residuo disponible (sin el "x3" mágico).
              const avgDisp = nDisp ? (g.metros_disponibles_cm || 0) / nDisp : 0;
              // Se topa en 600 para que nunca se desborde la barra de referencia.
              const avgCap  = Math.min(avgDisp, 600);
              const pct     = Math.round((avgCap / 600) * 100);
              return (
                <>
                  <SimuladorCortesCanvas
                    barra_cm={600}
                    piezas={[]}
                    residuos={avgCap > 0 ? [{ longitud: Math.round(avgCap), reutilizable: true }] : []}
                  />
                  <p style={{ fontSize: '.78rem', color: 'var(--text-muted)', marginTop: 10, textAlign: 'center' }}>
                    {nDisp > 0
                      ? `En promedio, un residuo disponible mide ${Math.round(avgDisp)} cm — el ${pct}% de una barra de 600 cm. Entre más grande, más fácil de reutilizar.`
                      : 'Aún no hay residuos disponibles para mostrar.'}
                  </p>
                </>
              );
            })()}
